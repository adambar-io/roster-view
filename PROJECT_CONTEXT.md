...
- Design previews that led here (mobile A/B, desktop two-pane, full-screen expand) were throwaway HTML files, not in the repo. Done: step 1 (tokens, type, theme toggle), step 2 (shell: sidebar / tab bar, summary cards, two-pane desktop, Compare + Settings screens). Planned next: roster as a real table on desktop + team-color accents + full-screen expand views with animated open/close (step 3), motion polish (step 4).
